...
class MesaModel {
  ...
  static async listarMesasDoMestre(mestreId) {
    try {
  ...
  static async atualizarVagasDisponiveis(mesa_id) {
    try {
      ...
      let { data, error } = await supabase.rpc("atualizarvagas", {
        mesa_id,
      });
  ...
  static async adicionarVaga(mesa_id) {
    try {
      //chamada da função do banco
      let { data, error } = await supabase.rpc("adicionarvaga", {
        mesa_id,
      });
