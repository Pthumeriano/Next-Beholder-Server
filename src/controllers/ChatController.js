...
const excluirChat = async (req, res) => {
    try {
        const result = await ChatService.excluirChat(req.body.id, req.usuarioAutenticado.userId);
...
const criarChat = async (req, res) => {
    try {
        ...
        const result = await ChatService.criarChat(req.usuarioAutenticado.userId);
